...
class MapRenderer : public sigc::trackable {
		...
		class ThreadImageBuffer : public sigc::trackable {
		  public:
			...
			bool need_glideslope(void) const { return m_need_glideslope; }
			void set_need_altitude(bool x = true) { m_need_altitude = x; }
			void set_need_glideslope(bool x = true) { m_need_glideslope = x; }
...
class VectorMapRenderer : public MapRenderer {
		...
		class DrawThread : public sigc::trackable {
				...
				void set_glideslope(float gs = std::numeric_limits<float>::quiet_NaN()) { m_buffer.set_glideslope(gs); }
				float get_glideslope(void) const { return m_buffer.get_glideslope(); }
				void set_tas(float tas = std::numeric_limits<float>::quiet_NaN()) { m_buffer.set_tas(tas); }
				float get_tas(void) const { return m_buffer.get_tas(); }
