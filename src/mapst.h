...
// C++ Interface: mapst
//
// Description: Maps Drawing, Threaded
//
//
// Author: Thomas Sailer <t.sailer@alumni.ethz.ch>, (C) 2007, 2008, 2012, 2013, 2015, 2017
//
...
public:
	...
	public:
		GlideModel(void);
		GlideModel(const Aircraft::ClimbDescent& cd);
		bool is_invalid(void) const;
		bool is_approxequal(const GlideModel& x) const;
		double compute_newalt(Wind<double>& wind, const Point& pt0, const Point& pt1, double alt0) const;
		...
		typedef Aircraft::Poly1D<double> poly_t;
	...
	public:
		...
		ImageBuffer(const ScreenCoord& imgsize, const Point& center, float_t nmi_per_pixel = 0.1, uint16_t upangle = 0,
			    int alt = 0, int maxalt = std::numeric_limits<int>::max(), int64_t time = 0, const GlideModel& glidemodel = GlideModel(),
			    float_t winddir = 0, float_t windspeed = 0, DrawFlags flags = drawflags_none, const ScreenCoord& offset = ScreenCoord(0, 0));
		...
		const GlideModel& get_glidemodel(void) const { return m_glidemodel; }
	...
	class ThreadImageBuffer : public sigc::trackable {
	public:
		ThreadImageBuffer(const ScreenCoord& scrsize = ScreenCoord(0, 0), const Point& center = Point(), int alt = 0, uint16_t upangle = 0,
				  int64_t time = 0, DrawFlags df = drawflags_none);

		bool need_altitude(void) const { return m_need_altitude; }
		bool need_glidemodel(void) const { return m_need_glidemodel; }
		void set_need_altitude(bool x = true) { m_need_altitude = x; }
		void set_need_glidemodel(bool x = true) { m_need_glidemodel = x; }
		...
		void set_scale(float nmi_per_pixel);
		...
		DrawFlags get_drawflags(void) const { return m_drawflags; }
		const GlideModel& get_glidemodel(void) const { return m_glidemodel; }
		void set_glidemodel(const GlideModel& gm = GlideModel());
		...
		// main thread functions to control draw thread
		Glib::Mutex& get_mutex(void) { return m_mutex; }
		void terminate(void);
		void restart(void);
		void restart_nolock(void);
		bool update(void);
		...
		int64_t screen_get_time(void) const { return m_screenbuf.get_time(); }
		...
		bool draw_asyncwait(void);
		bool draw_asyncwait_nolock(void);
		void draw_done(void);
		...
		const GlideModel& draw_get_glidemodel(void) const { return m_drawbuf.get_glidemodel(); }
	...
	protected:
		...
		bool m_hidden;
		bool m_need_altitude;
		bool m_need_glidemodel;
		...
		bool check_inside_nolock(const ImageBuffer<float>& imgbuf) const;
		void check_restart(void);
		bool update_nolock(void);
	...
	virtual void set_glidemodel(const GlideModel& gm = GlideModel()) = 0;
	virtual const GlideModel& get_glidemodel(void) const { return invalid_glidemodel; }
...
protected:
	static const GlideModel invalid_glidemodel;
	Point m_center;
	int64_t m_time;
	int m_altitude;
	uint16_t m_upangle;
...
private:
	...
	public:
		...
		void set_screensize(const ScreenCoord& scrsize) { m_buffer.set_screensize(scrsize); }
		const ScreenCoord& get_screensize(void) const { return m_buffer.get_screensize(); }
		void set_center(const Point& pt, int alt, uint16_t upangle, int64_t time);
		const Point& get_center(void) const { return m_buffer.get_center(); }
		void set_offset(const ScreenCoord& offs) { m_buffer.set_offset(offs); }
		const ScreenCoord& get_offset(void) const { return m_buffer.get_offset(); }
		int get_altitude(void) const { return m_buffer.get_altitude(); }
		uint16_t get_upangle(void) const { return m_buffer.get_upangle(); }
		int64_t get_time(void) const { return m_buffer.get_time(); }
		virtual void set_map_scale(float nmi_per_pixel);
		float get_map_scale(void) const { return m_buffer.get_scale(); }
		...
		void set_maxalt(int alt = std::numeric_limits<int>::max()) { m_buffer.set_maxalt(alt); }
		int get_maxalt(void) const { return m_buffer.get_maxalt(); }
		void set_glidemodel(const GlideModel& gm = GlideModel()) { m_buffer.set_glidemodel(gm); }
		const GlideModel& get_glidemodel(void) const { return m_buffer.get_glidemodel(); }
		void set_wind(float dir = 0, float speed = 0) { m_buffer.set_wind(dir, speed); }
		float get_winddir(void) const { return m_buffer.get_winddir(); }
		float get_windspeed(void) const { return m_buffer.get_windspeed(); }
		DrawFlags set_drawflags(DrawFlags df) { return m_buffer.set_drawflags(df); }
		DrawFlags get_drawflags(void) const { return m_buffer.get_drawflags(); }
		...
		void hide(void) { m_buffer.set_hidden(true); }
		void show(void) { m_buffer.set_hidden(false); }
		...
		Point screen_coord(const ScreenCoord& sc) const { return m_buffer(sc); }
		Point screen_coord(const ScreenCoordFloat& sc) const { return m_buffer(sc); }
		ScreenCoord coord_screen(const Point& pt) const { return to_screencoord(m_buffer(pt)); }
	...
	protected:
		...
		void async_done(void) { m_buffer.async_signal(); }
		void restart(void) { m_buffer.restart(); }
		void restart_db(void);

		virtual void compute_pixmap(bool dbquery) = 0;
	...
	protected:
		...
		virtual bool need_altitude(void) { return !!(m_buffer.draw_get_drawflags() & drawflags_weather); }
	...
	protected:
		...
		void async_cancel(void);
		void compute_pixmap(bool dbquery);
...
public:
	...
	virtual void set_glidemodel(const GlideModel& gm = GlideModel()) { if (m_thread) m_thread->set_glidemodel(gm); }
	virtual const GlideModel& get_glidemodel(void) const { return m_thread ? m_thread->get_glidemodel() : invalid_glidemodel; }
