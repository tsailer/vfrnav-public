...
VectorMapRenderer::DrawThread::DrawThread(Engine& eng, Glib::Dispatcher& dispatch, const ScreenCoord& scrsize, const Point& center, int alt, uint16_t upangle, int64_t time)
	  ...
	  m_center(center), m_screensize(scrsize), m_offset(0, 0), m_drawoffset(0, 0), m_time(time), m_scale(0.1), m_glideslope(std::numeric_limits<float>::quiet_NaN()),
...
void VectorMapRenderer::DrawThread::set_glideslope(float gs)
{
	if (gs <= 0)
		gs = std::numeric_limits<float>::quiet_NaN();
	m_glideslope = gs;
...
void VectorMapRenderer::DrawThread::set_tas(float tas)
{
	if (tas <= 0)
		tas = std::numeric_limits<float>::quiet_NaN();
	m_tas = tas;
...
void VectorMapRenderer::DrawThread::restart(void)
{
	...
	m_drawbuf.reset(ScreenCoord(m_screensize.getx() * 2, m_screensize.gety() * 2), m_center, m_scale, m_upangle, m_altitude, m_maxalt,
			m_time, m_glideslope, m_tas, m_winddir, m_windspeed, m_drawflags, m_offset);
	bool dbquery(false);
	{
...
void VectorMapRenderer::DrawThreadTerrain::draw_glidearea(const Cairo::RefPtr<Cairo::ImageSurface> surface)
{
	...
	// FIXME: Consider wind!
	float glideslope(draw_get_glideslope());
	...
	workqueue_t workqueue;
	{
		...
		}
		{
			...
			workqueue.push_back(coordalt_t(coord_t(pt.get_lon(), pt.get_lat()), draw_get_altitude() * Point::ft_to_m));
			if (false)
