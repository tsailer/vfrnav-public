...
const uint16_t FPlanWaypoint::altflag_generated;
...
const std::string FPlanWaypoint::reader_fields = "ICAO,NAME,LON,LAT,ALT,FREQ,TIME,SAVETIME,FLAGS,PATHNAME,PATHCODE,NOTE,WINDDIR,WINDSPEED,QFF,ISAOFFS,"
	"TRUEALT,TERRAIN,TRUETRACK,TRUEHEADING,DECLINATION,DIST,FUEL,TAS,RPM,MP,TYPE,NAVAID,FLIGHTTIME,MASS,ENGINEPROFILE,BHP";
...
FPlanWaypoint::FPlanWaypoint(void)
	  ...
	  m_truealt(0), m_terrain(invalid_altitude), m_dist(0), m_mass(0), m_fuel(0), m_bhp(0), m_flags(0), m_winddir(0), m_windspeed(0),
...
FPlanWaypoint::FPlanWaypoint(sqlite3x::sqlite3_cursor& cursor)
	  ...
	  m_truealt(cursor.getint(16)), m_terrain(cursor.getint(17)), m_dist(cursor.getint(21)), m_mass(cursor.getint(29)),
...
FPlanAlternate::FPlanAlternate(const FPlanWaypoint& w)
	: FPlanWaypoint(w), m_holdtime(0), m_holdfuel(0)
{
}
        ...
        for (unsigned int nr = 0; nr < get_nrwpt(); nr++) {
                ...
                }
                {
                        sqlite3x::sqlite3_command cmd(m_fp->get_db(), "UPDATE waypoints SET ICAO  = ?, NAME = ?, "
						      ...
						      "FLIGHTTIME = ?, MASS = ?, ENGINEPROFILE = ?, BHP = ? "
						      "WHERE PLANID = ? AND NR = ?;");
					  ...
					  const Aircraft::Cruise::EngineParams& ep, bool insert_tocbod)
{
	...
	Profile p;
	unsigned int centeridx((get_nrwpt() == 2 && !(*this)[0].get_coord().is_invalid() && !(*this)[1].get_coord().is_invalid()) ? 1U : ~0U);
	if (centeridx != ~0U) {
		...
		wptcenter.set_coord(wptp.get_coord().halfway(wptn.get_coord()));
		{
			Wind<float> wnd;
			wnd.set_wind(wptp.get_winddir_rad(), wptp.get_windspeed_kts(),
				     wptn.get_winddir_rad(), wptn.get_windspeed_kts());
			wptcenter.set_winddir_rad(wnd.get_winddir());
			wptcenter.set_windspeed_kts(wnd.get_windspeed());
		}
		wptcenter.set_qff_hpa(0.5 * (wptp.get_qff_hpa() + wptn.get_qff_hpa()));
		wptcenter.set_isaoffset_kelvin(0.5 * (wptp.get_isaoffset_kelvin() + wptn.get_isaoffset_kelvin()));
		wptcenter.set_declination_rad(wptp.get_declination_rad() + 0.5 * (wptn.get_declination_rad() - wptp.get_declination_rad()));
		if (alt != FPlanWaypoint::invalid_altitude && alt != 0) {
			wptcenter.frob_flags(flags & FPlanWaypoint::altflag_standard, FPlanWaypoint::altflag_standard);
			wptcenter.set_altitude(alt);
		} else if (IcaoFlightPlan::is_route_pogo(wptp.get_icao(), wptn.get_icao())) {
			...
			for (; pg->icao; ++pg)
				if (wptp.get_icao() == pg->icao)
					break;
			wptcenter.frob_flags(FPlanWaypoint::altflag_standard, FPlanWaypoint::altflag_standard);
			wptcenter.set_altitude(pg->alt);
		} else {
			double dist(wptp.get_coord().spheric_distance_nmi_dbl(wptn.get_coord()));
			int32_t alt(std::max(wptp.get_altitude(), wptn.get_altitude()) + 1000);
			if (!wptn.is_ifr())
				alt += 500;
			alt = ((alt + 999) / 1000) * 1000;
			if (!wptn.is_ifr())
				alt -= 500;
			...
			if (std::isnan(qnh3))
				qnh3 = wptcenter.get_qff() ? wptcenter.get_qff_hpa() : IcaoAtmosphere<float>::std_sealevel_pressure;
			if (std::isnan(tempoffs3))
				tempoffs3 = wptcenter.get_isaoffset_kelvin();
	...
	for (unsigned int i(0), n(get_nrwpt() + (centeridx != ~0U)); i < n;) {
		unsigned int i0(i);
		for (;;) {
			++i;
			if (i >= n)
				break;
			const FPlanWaypoint& wpt((i == centeridx) ? wptcenter : (*this)[i - (i > centeridx)]);
			if (!wpt.get_coord().is_invalid())
				break;
		}
		if (i >= n)
			break;
		FPlanWaypoint *wpt((i == centeridx) ? &wptcenter : &(*this)[i - (i > centeridx)]);
		FPlanWaypoint *wpto((i0 == centeridx) ? &wptcenter : &(*this)[i0 - (i > centeridx)]);
		...
		if (i != centeridx && 
			...
			p.set_distat(i - (i > centeridx), cumdist + dist);
		}
		if (wpt->is_standard())
			ad.set_pressure_altitude(wpt->get_altitude());
		else
		...
		if (newmode == mode_climb) {
			...
			if (false)
				climb.print(std::cerr << "FPlanRoute::recompute: climb" << std::endl, "  ") << std::endl;
			...
			if (climbd <= dist + 1.0) {
				...
				if (dist <= 1.0) {
					dist = 0;
				} else if (insert_tocbod && !stay && centeridx == ~0U) {
					...
					wptx.frob_flags(((wptx.get_flags() ^ wpt->get_flags()) & FPlanWaypoint::altflag_standard) | FPlanWaypoint::altflag_generated,
							FPlanWaypoint::altflag_climb | FPlanWaypoint::altflag_descent | FPlanWaypoint::altflag_generated);
					...
					insert_wpt(i, wptx);
					++n;
					wpt = (i == centeridx) ? &wptcenter : &(*this)[i - (i > centeridx)];
					wpto = (i0 == centeridx) ? &wptcenter : &(*this)[i0 - (i > centeridx)];
					dist = 0;
				}
			} else {
		...
		if (newmode == mode_descent) {
			...
			if (tas < 1)
				throw std::runtime_error("FPlanRoute::recompute: invalid aircraft model");
			if (wind.get_windspeed() > 0) {
				wind.set_crs_tas(tt, tas);
				if (std::isnan(wind.get_gs()) || std::isnan(wind.get_hdg()) || wind.get_gs() <= 0) {
					descentd = 0;
			...
			if (insert_tocbod && i + 1 >= n && descentd <= dist - 1.0 && centeridx == ~0U) {
				newmode = mode_cruise;
				nextpa = prevpa;
				dist -= descentd;
				FPlanWaypoint wptx(*wpto);
				...
				wptx.frob_flags(((wptx.get_flags() ^ wpto->get_flags()) & FPlanWaypoint::altflag_standard) | FPlanWaypoint::altflag_generated,
						FPlanWaypoint::altflag_climb | FPlanWaypoint::altflag_descent | FPlanWaypoint::altflag_generated);
				wptx.set_frequency(0);
				wptx.set_tas(0);
				wptx.set_rpm(0);
				...
				wpto->set_tas_kts(tas);
				if (descentd <= dist + 1.0 || ((wpto->get_flags() & FPlanWaypoint::altflag_generated) && wpto->get_name() == "*TOD*")) {
		...
		if (mode == mode_cruise && (stay || dist > 0)) {
			...
			if (std::isnan(isaoffs1))
				isaoffs1 = wpto->get_isaoffset_kelvin();
			if (false)
				ep1.print(std::cerr << "Cruise: before: TAS " << tas << " FF " << fuelflow << " PA " << pa << " mass " << mass1 << " isaoffs " << isaoffs1
					  << " qnh " << qnh1 << " ep ") << " wpt rpm " << wpt->get_rpm() << " mp " << wpt->get_mp() << std::endl;
			acft.calculate_cruise(tas, fuelflow, pa, mass1, isaoffs1, qnh1, ep1);
			if (false)
				ep1.print(std::cerr << "Cruise: after: TAS " << tas << " FF " << fuelflow << " PA " << pa << " mass " << mass1 << " isaoffs " << isaoffs1
					  << " qnh " << qnh1 << " ep ") << std::endl;
			if (tas < 1)
				throw std::runtime_error("FPlanRoute::recompute: invalid aircraft model");
			...
			if (stay) {
				...
				if (wind.get_windspeed() > 0) {
					...
					if (std::isnan(wind.get_gs()) || std::isnan(wind.get_hdg()) || wind.get_gs() <= 0) {
						gs = 1;
					} else if (!wpto->is_climb()) {
						th = wind.get_hdg();
						gs = wind.get_gs();
					}
					if (false)
						std::cerr << "FPlanRoute::recompute: cruise: windcorr: TT " << tt << " TH " << th
	...
	if (centeridx <= get_nrwpt() && centeridx > 0) {
		FPlanWaypoint& wpt((*this)[centeridx - 1U]);
		wpt.set_dist(wpt.get_dist() + wptcenter.get_dist());
	}
	if (false) {
		for (unsigned int i(0), n(get_nrwpt()); i < n; ++i) {
			const FPlanWaypoint& wpt((*this)[i]);
			std::ostringstream oss;
			oss << "FPL[" << i << "]: " << wpt.get_icao() << '/' << wpt.get_name() << ' ';
			...
			oss << ' ' << wpt.get_coord().get_lat_str2() << ' ' << wpt.get_coord().get_lon_str2()
			    ...
			    << ' ' << wpt.get_truealt() << "' flags 0x"
			    << std::hex << std::setfill('0') << std::setw(4) << wpt.get_flags() << std::dec;
