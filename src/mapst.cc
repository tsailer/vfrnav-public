...
void MapRenderer::ThreadImageBuffer::set_center(const Point& pt)
{
	{
...
void MapRenderer::ThreadImageBuffer::set_center(const Point& pt, int alt, uint16_t upangle, int64_t time)
{
	{
		Glib::Mutex::Lock lock(m_mutex);
		m_center = pt;
...
void MapRenderer::ThreadImageBuffer::set_screensize(const ScreenCoord& scrsize)
{
	{
		Glib::Mutex::Lock lock(m_mutex);
...
void MapRenderer::ThreadImageBuffer::set_offset(const ScreenCoord& offs)
{
	{
		Glib::Mutex::Lock lock(m_mutex);
		m_offset = offs;
...
void MapRenderer::ThreadImageBuffer::set_upangle(uint16_t upangle)
{
        Glib::Mutex::Lock lock(m_mutex);
	m_upangle = upangle;
	check_restart();
...
void MapRenderer::ThreadImageBuffer::set_time(int64_t time)
{
        Glib::Mutex::Lock lock(m_mutex);
	m_time = time;
	check_restart();
...
void MapRenderer::ThreadImageBuffer::set_scale(float nmi_per_pixel)
{
        Glib::Mutex::Lock lock(m_mutex);
...
void MapRenderer::ThreadImageBuffer::set_altitude(int alt)
{
	{
		Glib::Mutex::Lock lock(m_mutex);
		m_altitude = alt;
...
void MapRenderer::ThreadImageBuffer::set_wind(float winddir, float windspeed)
{
...
MapRenderer::DrawFlags MapRenderer::ThreadImageBuffer::set_drawflags(DrawFlags df)
{
	{
		Glib::Mutex::Lock lock(m_mutex);
		m_drawflags = df;
...
void MapRenderer::ThreadImageBuffer::set_hidden(bool hidden)
{
	Glib::Mutex::Lock lock(m_mutex);
...
void MapRenderer::ThreadImageBuffer::terminate(void)
{
	Glib::Mutex::Lock lock(m_mutex);
...
void MapRenderer::ThreadImageBuffer::restart(void)
{
	Glib::Mutex::Lock lock(m_mutex);
	restart_nolock();
}

bool MapRenderer::ThreadImageBuffer::update(void)
{
	bool r;
	{
		Glib::Mutex::Lock lock(m_mutex);
...
bool MapRenderer::ThreadImageBuffer::draw_wait(void)
{
	ScreenCoord imgsize(0, 0);
	ScreenCoord offset(0, 0);
	Point center;
	float scale;
	uint16_t upangle;
	int64_t time;
	...
	DrawFlags df;
	{
		...
		for (;;) {
			if (true)
				std::cerr << "draw_wait: threadstate: " << m_threadstate << std::endl;
			if (m_threadstate == thread_busy) {
				if (true)
					std::cerr << "draw_wait: abort" << std::endl;
				m_threadstate = thread_idle;
			}
			if (m_threadstate == thread_terminate)
				return true;
			if (!m_hidden && (m_threadstate != thread_idle && m_threadstate != thread_ready))
				break;
			m_cond.wait(m_mutex);
		}
		m_threadstate = thread_busy;
		imgsize = ScreenCoord(m_screensize.getx() * 2, m_screensize.gety() * 2);
		offset = m_offset;
		center = m_center;
		scale = m_scale;
		upangle = m_upangle;
...
bool MapRenderer::ThreadImageBuffer::draw_checkabort(void)
{
        Glib::Mutex::Lock lock(m_mutex);
	if (false)
		std::cerr << "draw_checkabort: threadstate: " << m_threadstate << std::endl;
	return (m_threadstate == thread_restart || m_threadstate == thread_terminate || m_hidden);
}

void MapRenderer::ThreadImageBuffer::draw_done(void)
{
	bool doflush(false);
	{
		Glib::Mutex::Lock lock(m_mutex);
		doflush = m_threadstate == thread_busy && m_drawbuf;
	}
	if (doflush)
	...
	if (m_threadstate == thread_busy) {
		if (true)
			std::cerr << "draw_done: mark done" << std::endl;
		m_threadstate = thread_ready;
	}
}

bool MapRenderer::ThreadImageBuffer::draw_asyncwait_nolock(void)
{
	if (true)
		std::cerr << "draw_asyncwait_nolock: threadstate: " << m_threadstate << std::endl;
	if (m_threadstate == thread_restart || m_threadstate == thread_terminate || m_hidden) {
		if (true)
			std::cerr << "draw_asyncwait_nolock: abort" << std::endl;
		return true;
	}
	m_cond.wait(m_mutex);
	if (true)
...
bool MapRenderer::ThreadImageBuffer::draw_asyncwait(void)
{
        Glib::Mutex::Lock lock(m_mutex);
	return draw_asyncwait_nolock();
}

void MapRenderer::ThreadImageBuffer::async_signal(void)
{
	if (true)
		std::cerr << "async_signal" << std::endl;
        Glib::Mutex::Lock lock(m_mutex);
	m_cond.broadcast();
}

bool MapRenderer::ThreadImageBuffer::update_nolock(void)
{
	if (m_threadstate != thread_ready)
		return false;
	...
	if (true)
		std::cerr << "update: new screen image: size " << m_screenbuf.get_imagesize() << " center " << m_screenbuf.get_imagecenter()
			  << " scale " << get_scale() << " north " << m_screenbuf.get_upangle() << " alt " << m_screenbuf.get_altitude()
			  << " maxalt " << get_maxalt() << " flags " << m_screenbuf.get_drawflags() << std::endl;
	return true;
}

void MapRenderer::ThreadImageBuffer::restart_nolock(void)
{
	if (m_threadstate == thread_terminate)
		return;
	m_threadstate = thread_restart;
	m_cond.broadcast();
}
...
void MapRenderer::ThreadImageBuffer::check_restart(void)
{
	...
	if (false) {
		...
		std::cerr << "ThreadImageBuffer::check_restart: hidden " << is_hidden()
			  ...
			  << " scale " << check_window((float)(get_scale() / m_screenbuf.get_scale()), (float)(1. / (1 + map_tolerance)), (float)(1 + map_tolerance))
			  << " inside " << check_inside_nolock(m_screenbuf) << " gsinside " << gsrect.is_inside(m_screenbuf.get_center()) << std::endl
			  ...
			  << " scale " << check_window((float)(get_scale() / m_drawbuf.get_scale()), (float)(1. / (1 + map_tolerance)), (float)(1 + map_tolerance))
			  << " inside " << check_inside_nolock(m_drawbuf) << " gsinside " << gsrect.is_inside(m_drawbuf.get_center()) << std::endl;
	}
	if (is_hidden())
		return;
	Rect gsrect;
...
MapRenderer::ScreenCoord MapRenderer::ThreadImageBuffer::calc_image_drawoffset(const ImageBuffer<float>& imgbuf) const
{
	return to_screencoord(imgbuf(get_center()) - get_screencenter()) + get_offset();
}

void MapRenderer::ThreadImageBuffer::update_drawoffset(void)
{
	m_drawoffset = calc_image_drawoffset(m_screenbuf);
}

MapRenderer::ScreenCoordFloat MapRenderer::ThreadImageBuffer::operator()(const Point& pt) const
{
	return m_screenbuf(pt) - to_screencoordfloat(m_drawoffset);
}

Point MapRenderer::ThreadImageBuffer::operator()(float_t x, float_t y) const
{
	return m_screenbuf(x + m_drawoffset.getx(), y + m_drawoffset.gety());
}

Point MapRenderer::ThreadImageBuffer::operator()(const ScreenCoord& sc) const
{
	return (*this)(sc.getx(), sc.gety());
}

Point MapRenderer::ThreadImageBuffer::operator()(const ScreenCoordFloat& sc) const
{
	return (*this)(sc.getx(), sc.gety());
}

bool MapRenderer::ThreadImageBuffer::check_inside_nolock(const ImageBuffer<float>& imgbuf) const
{
...
void MapRenderer::ThreadImageBuffer::draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
	update();
	if (false)
...
VectorMapRenderer::DrawThread::DrawThread(Engine& eng, Glib::Dispatcher& dispatch, const ScreenCoord& scrsize, const Point& center, int alt, uint16_t upangle, int64_t time)
        : m_buffer(scrsize, center, alt, upangle, time), m_engine(eng), m_thread(0), m_dispatch(dispatch), m_dbrectangle(empty_rect)
{
...
void VectorMapRenderer::DrawThread::stop(void)
{
	if (!m_thread)
		return;
	m_buffer.terminate();
...
{
	m_buffer.set_center(pt, alt, upangle, time);
}
...
void VectorMapRenderer::DrawThread::set_map_scale(float nmi_per_pixel)
{
	...
	m_buffer.set_scale(nmi_per_pixel);
}
...
{
        if (m_buffer.get_drawflags() & drawflags_airspaces)
		restart_db();
}

void VectorMapRenderer::DrawThread::airports_changed(void)
{
        if (m_buffer.get_drawflags() & drawflags_airports)
		restart_db();
}

void VectorMapRenderer::DrawThread::navaids_changed(void)
{
        if (m_buffer.get_drawflags() & drawflags_navaids)
		restart_db();
}

void VectorMapRenderer::DrawThread::waypoints_changed(void)
{
        if (m_buffer.get_drawflags() & drawflags_waypoints)
		restart_db();
}

void VectorMapRenderer::DrawThread::airways_changed(void)
{
        if (m_buffer.get_drawflags() & (drawflags_airways_low | drawflags_airways_high))
		restart_db();
}

void VectorMapRenderer::DrawThread::thread(void)
{
        for (;;) {
		if (true)
			std::cerr << "draw thread: waiting" << std::endl;
		if (m_buffer.draw_wait())
			break;
		bool dbquery(false);
		{
			Rect rect(m_buffer.draw_coverrect(2, 2));
			dbquery = !m_dbrectangle.is_inside(rect);
			if (dbquery)
				recompute_dbrectangle();
		}
		if (true)
			std::cerr << "draw thread: action: dbquery " << dbquery << " hidden " << m_buffer.is_hidden() << std::endl;
		compute_pixmap(dbquery);
		m_dispatch();
        }
	if (true)
		std::cerr << "draw thread: terminating" << std::endl;
}

void VectorMapRenderer::DrawThread::recompute_dbrectangle(void)
{
	m_dbrectangle = m_buffer.draw_coverrect(3, 3);
}

void VectorMapRenderer::DrawThread::restart_db(void)
{
	Glib::Mutex::Lock lock(m_buffer.get_mutex());
	clear_dbrectangle();
	m_buffer.restart_nolock();
}
...
void VectorMapRenderer::DrawThread::draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
	...
	if (m_grib2surface) {
		...
		cr->set_font_size(12);
		cr->translate(m_buffer.get_screensize().getx() - 10, 10);
		m_grib2surface->draw_scale(cr);
		cr->restore();
	}
	if (m_buffer.get_drawflags() & drawflags_weather) {
		cr->save();
		cr->select_font_face("Sans", Cairo::FONT_SLANT_NORMAL, Cairo::FONT_WEIGHT_NORMAL);
		cr->set_font_size(12);
		cr->translate(m_buffer.get_screensize().getx() - 10, m_buffer.get_screensize().gety() - 10);
...
{
        return nmi / m_buffer.draw_get_scale();
}
...
void VectorMapRenderer::DrawThread::polypath(Cairo::Context *cr, const PolygonSimple& poly, bool closeit)
{
        if (poly.empty())
                return;
	m_buffer.draw_path(cr, poly.begin(), poly.end());
        if (closeit)
                cr->close_path();
}
...
void VectorMapRenderer::DrawThread::draw_surface(Cairo::Context *cr, const Rect& cbbox, const Cairo::RefPtr<Cairo::ImageSurface>& pbuf, double alpha)
{
	...
	Cairo::Matrix matrix(m_buffer.draw_cairo_matrix(psz));
	...
	if (false)
		std::cerr << "draw_surface: matrix [" << matrix.xx << ' ' << matrix.xy << ' ' << matrix.x0 << " ; "
			  << matrix.yx << ' ' << matrix.yy << ' ' << matrix.y0 << " ] size " << pbufx << " / " << pbufy << std::endl;
	if (false)
		std::cerr << "draw_surface: cbbox " << cbbox << " center " << m_buffer.draw_get_imagecenter() << std::endl;
 	if (false)
		std::cerr << "draw_surface: " << cbbox.get_northwest() << " -> " << m_buffer.draw_transform(cbbox.get_northwest()) << std::endl
			  << "    " << cbbox.get_southeast() << " -> " << m_buffer.draw_transform(cbbox.get_southeast()) << std::endl;
 	if (false) {
...
void VectorMapRenderer::DrawThread::draw_pixbuf(const Cairo::RefPtr<Cairo::Context>& cr, const Rect& cbbox, const Glib::RefPtr<Gdk::Pixbuf>& pbuf, double alpha)
{
	...
	if (false)
		std::cerr << "draw_pixbuf: matrix [" << matrix.xx << ' ' << matrix.xy << ' ' << matrix.x0 << " ; "
			  << matrix.yx << ' ' << matrix.yy << ' ' << matrix.y0 << " ] size " << pbufx << " / " << pbufy << std::endl;
	if (false)
		std::cerr << "draw_pixbuf: cbbox " << cbbox << " center " << m_buffer.draw_get_imagecenter() << std::endl;
 	if (false)
		std::cerr << "draw_pixbuf: " << cbbox.get_northwest() << " -> " << m_buffer.draw_transform(cbbox.get_northwest()) << std::endl
			  << "    " << cbbox.get_southeast() << " -> " << m_buffer.draw_transform(cbbox.get_southeast()) << std::endl;
 	if (false) {
		double x0(0), y0(0), x1(pbufx), y1(pbufy);
		cairo_matrix_transform_point(&matrix, &x0, &y0);
...
void VectorMapRenderer::DrawThread::draw_pixbuf(const Cairo::RefPtr<Cairo::Context>& cr, const Corner& corner, const Glib::RefPtr<Gdk::Pixbuf>& pbuf, double alpha)
{
	if (true) {
		...
		matrix.xx = matrix.xy = matrix.yx = matrix.yy = matrix.x0 = matrix.y0 = 0;
		{
			...
			for (unsigned int i = 0; i < 4; ++i) {
				ScreenCoordFloat sc(m_buffer.draw_transform(corner[i]));
		...
		if (false)
			std::cerr << "draw_pixbuf: matrix [" << matrix.xx << ' ' << matrix.xy << ' ' << matrix.x0 << " ; "
				  << matrix.yx << ' ' << matrix.yy << ' ' << matrix.y0 << " ] size " << pbufx << " / " << pbufy << std::endl;
		if (false)
			std::cerr << "draw_pixbuf: " << corner[0] << " -> " << m_buffer.draw_transform(corner[0]) << std::endl
				  << "    " << corner[1] << " -> " << m_buffer.draw_transform(corner[1])
				  << "    " << corner[2] << " -> " << m_buffer.draw_transform(corner[2])
				  << "    " << corner[3] << " -> " << m_buffer.draw_transform(corner[3]) << std::endl;
		if (false) {
			double x0(0), y0(0), x1(pbufx), y1(pbufy);
			cairo_matrix_transform_point(&matrix, &x0, &y0);
			cairo_matrix_transform_point(&matrix, &x1, &y1);
...
{
	float scale(m_buffer.draw_get_scale());
...
{
	cr->rotate(m_buffer.screen_get_upangle() * -to_rad_16bit_dbl);
	cr->move_to(-0.5, 1);
	cr->line_to(-0.5, -1);
	cr->line_to(0.5, 1);
	cr->line_to(0.5, -1);
...
void VectorMapRenderer::DrawThread::draw_route(Cairo::Context *cr, const FPlanRoute *route)
{
	...
	for (unsigned int i = 0; i < coords.size(); ++i)
		coords[i] = m_buffer((*route)[i].get_coord());
	{
...
void VectorMapRenderer::DrawThread::draw_track(Cairo::Context *cr, const TracksDb::Track *track)
{
		...
		cairo_matrix_translate(&matrix, patw/2, path/2);
			...
			ScreenCoordFloat px(m_buffer((*track)[i].get_coord()));
	...
	m_buffer.move_to(cr, (*track)[0].get_coord());
        for (unsigned int i = 1; i < track->size(); ++i)
		m_buffer.line_to(cr, (*track)[i].get_coord());
        cr->stroke();
        cr->restore();
}
...
void VectorMapRenderer::DrawThread::draw_label(Cairo::Context *cr, const Glib::ustring& text, const Point& pt, NavaidsDb::Navaid::label_placement_t lp, int dist)
{
        if (text.empty() || lp == NavaidsDb::Navaid::label_off)
                return;
        draw_label(cr, text, m_buffer.draw_transform(pt), lp, dist, m_buffer.draw_get_upangle());
}
...
void VectorMapRenderer::DrawThread::draw_waypoint(Cairo::Context *cr, const Point& pt, const Glib::ustring& str, bool mandatory, NavaidsDb::Navaid::label_placement_t lp, bool setcolor)
{
	...
	if (setcolor)
        ...
        ScreenCoordFloat px(m_buffer.draw_transform(pt));
...
void VectorMapRenderer::DrawThread::draw_navaid(Cairo::Context *cr, const NavaidsDb::Navaid& n, const ScreenCoordFloat& pos)
{
        ...
        switch (t) {
			...
			if (t == NavaidsDb::Navaid::navaid_vortac || t == NavaidsDb::Navaid::navaid_tacan) {
				...
				cr->rotate(m_buffer.draw_get_upangle() * -to_rad_16bit_dbl);
        ...
        draw_label(cr, n.get_icao() + ' ' + freqstr(n.get_frequency()), pos, n.get_label_placement(), dist, m_buffer.draw_get_upangle());
}
...
{
	return ((m_buffer.draw_get_drawflags() & drawflags_navaids) && (!m_navaidquery || m_navaidquery->is_error()))
		|| ((m_buffer.draw_get_drawflags() & drawflags_waypoints) && (!m_waypointquery || m_waypointquery->is_error()))
		|| ((m_buffer.draw_get_drawflags() & drawflags_airports) && (!m_airportquery || m_airportquery->is_error()))
		|| ((m_buffer.draw_get_drawflags() & drawflags_airspaces) && (!m_airspacequery || m_airspacequery->is_error()))
		|| ((m_buffer.draw_get_drawflags() & (drawflags_airways_low | drawflags_airways_high)) && (!m_airwayquery || m_airwayquery->is_error()));
}

void VectorMapRenderer::DrawThreadOverlay::db_restart(void)
{
	async_cancel();
	if (m_buffer.draw_get_drawflags() & drawflags_navaids)
		m_navaidquery = m_engine.async_navaid_find_bbox(get_dbrectangle(), ~0, NavaidsDb::element_t::subtables_all);
	if (m_buffer.draw_get_drawflags() & drawflags_waypoints)
		m_waypointquery = m_engine.async_waypoint_find_bbox(get_dbrectangle(), ~0, WaypointsDb::element_t::subtables_all);
	if (m_buffer.draw_get_drawflags() & drawflags_airports)
	...
	if (m_buffer.draw_get_drawflags() & drawflags_airspaces)
		m_airspacequery = m_engine.async_airspace_find_bbox(get_dbrectangle(), ~0, AirspacesDb::element_t::subtables_none);
	if (m_buffer.draw_get_drawflags() & (drawflags_airways_low | drawflags_airways_high))
	...
	if (m_buffer.draw_get_drawflags() & drawflags_weather) {
		m_wxwindu.reset();
		m_wxwindv.reset();
		m_wxtemp.reset();
		m_wxqff.reset();
		...
		m_wxcldconvtop.reset();
		float press(0);
		IcaoAtmosphere<float>::std_altitude_to_pressure(&press, 0, m_buffer.draw_get_altitude() * Point::ft_to_m);
		press *= 100;
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_momentum_ugrd),
										  m_buffer.draw_get_time(), GRIB2::surface_isobaric_surface, press));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxwindu = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			...
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxwindv = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_temperature_tmp),
										  m_buffer.draw_get_time(), GRIB2::surface_isobaric_surface, press));
			std::cerr << "WX: Temp EffTime " << Glib::DateTime::create_now_utc(m_buffer.draw_get_time()).format("%F %H:%M:%S")
				  << " press " << press << "Pa" << std::endl;
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxtemp = x->get_results(m_buffer.draw_get_time(), press);
			if (m_wxtemp)
		...
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_mass_prmsl),
										  m_buffer.draw_get_time()));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time()));
			if (x)
				m_wxqff = x->get_results(m_buffer.draw_get_time());
		}
		{
			...
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxrh = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			...
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time()));
			if (x)
				m_wxprecip = x->get_results(m_buffer.draw_get_time());
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_cloud_tcdc),
										  m_buffer.draw_get_time(), GRIB2::surface_boundary_layer_cloud, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldbdrycover = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_cloud_tcdc),
										  m_buffer.draw_get_time(), GRIB2::surface_low_cloud, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldlowcover = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_mass_pres),
										  m_buffer.draw_get_time(), GRIB2::surface_low_cloud_bottom, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldlowbase = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_mass_pres),
										  m_buffer.draw_get_time(), GRIB2::surface_low_cloud_top, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldlowtop = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_cloud_tcdc),
										  m_buffer.draw_get_time(), GRIB2::surface_middle_cloud, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldmidcover = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_mass_pres),
										  m_buffer.draw_get_time(), GRIB2::surface_middle_cloud_bottom, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldmidbase = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_mass_pres),
										  m_buffer.draw_get_time(), GRIB2::surface_middle_cloud_top, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldmidtop = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_cloud_tcdc),
										  m_buffer.draw_get_time(), GRIB2::surface_top_cloud, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldhighcover = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_mass_pres),
										  m_buffer.draw_get_time(), GRIB2::surface_top_cloud_bottom, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldhighbase = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_mass_pres),
										  m_buffer.draw_get_time(), GRIB2::surface_top_cloud_top, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldhightop = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_cloud_tcdc),
										  m_buffer.draw_get_time(), GRIB2::surface_convective_cloud, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldconvcover = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_mass_pres),
										  m_buffer.draw_get_time(), GRIB2::surface_convective_cloud_bottom, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldconvbase = x->get_results(m_buffer.draw_get_time(), press);
		}
		{
			GRIB2::layerlist_t ll(m_engine.get_grib2_db().find_layers(GRIB2::find_parameter(GRIB2::param_meteorology_mass_pres),
										  m_buffer.draw_get_time(), GRIB2::surface_convective_cloud_top, 0));
			Glib::RefPtr<GRIB2::LayerInterpolateResult> x(GRIB2::interpolate_results(get_dbrectangle(), ll, m_buffer.draw_get_time(), press));
			if (x)
				m_wxcldconvtop = x->get_results(m_buffer.draw_get_time(), press);
		}
	}
...
{
	TimeMeasurement tmquery;
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
		for (;;) {
			if (!m_navaidquery || m_navaidquery->is_done())
				break;
			if (m_buffer.draw_asyncwait_nolock())
				return false;
		}
        }
        if ((m_buffer.draw_get_drawflags() & drawflags_navaids) && m_navaidquery && m_navaidquery->is_done() && !m_navaidquery->is_error()) {
                std::cerr << "navaids: " << m_navaidquery->get_result().size() << " time " << tmquery << std::endl;
                TimeMeasurement tm;
...
bool VectorMapRenderer::DrawThreadOverlay::do_draw_waypoints(Cairo::Context *cr)
{
	TimeMeasurement tmquery;
        ...
        if ((m_buffer.draw_get_drawflags() & drawflags_waypoints) && m_waypointquery && m_waypointquery->is_done() && !m_waypointquery->is_error()) {
                std::cerr << "waypoints: " << m_waypointquery->get_result().size() << " time " << tmquery << std::endl;
                TimeMeasurement tm;
                draw(cr, m_waypointquery->get_result());
                std::cerr << "waypoints drawing time " << tm << std::endl;
        }
	return true;
}
...
bool VectorMapRenderer::DrawThreadOverlay::do_draw_airports(Cairo::Context *cr)
{
	TimeMeasurement tmquery;
        ...
        if ((m_buffer.draw_get_drawflags() & drawflags_airports) && m_airportquery && m_airportquery->is_done() && !m_airportquery->is_error()) {
                std::cerr << "airports: " << m_airportquery->get_result().size() << " time " << tmquery << std::endl;
                TimeMeasurement tm;
                draw(cr, m_airportquery->get_result());
                std::cerr << "airports drawing time " << tm << std::endl;
        }
	return true;
}
...
bool VectorMapRenderer::DrawThreadOverlay::do_draw_airspaces(Cairo::Context *cr)
{
	TimeMeasurement tmquery;
        ...
        if ((m_buffer.draw_get_drawflags() & drawflags_airspaces) && m_airspacequery && m_airspacequery->is_done() && !m_airspacequery->is_error()) {
                std::cerr << "airspaces: " << m_airspacequery->get_result().size() << " time " << tmquery << std::endl;
                TimeMeasurement tm;
                draw(cr, m_airspacequery->get_result());
                std::cerr << "airspaces drawing time " << tm << std::endl;
        }
	return true;
}
...
bool VectorMapRenderer::DrawThreadOverlay::do_draw_airways(Cairo::Context *cr)
{
	TimeMeasurement tmquery;
        ...
        if ((m_buffer.draw_get_drawflags() & (drawflags_airways_low | drawflags_airways_high)) && !m_buffer.draw_checkabort() && m_airwayquery && m_airwayquery->is_done() && !m_airwayquery->is_error()) {
                std::cerr << "airways: " << m_airwayquery->get_result().size() << " time " << tmquery << std::endl;
                TimeMeasurement tm;
                draw(cr, m_airwayquery->get_result());
                std::cerr << "airways drawing time " << tm << std::endl;
        }
	return true;
}
...
{
	if (m_buffer.draw_get_drawflags() & drawflags_weather && m_wxwindu && m_wxwindv) {
		TimeMeasurement tm;
		draw_weather(cr);
		...
		std::cerr << "weather: " << m_wxwindu->get_width() << 'x' << m_wxwindu->get_height() << " scale " << m_buffer.draw_get_scale() << " drawing time " << tm << std::endl;
	} else {
...
void VectorMapRenderer::DrawThreadOverlay::draw(Cairo::Context *cr, const std::vector<NavaidsDb::Navaid>& navaids)
{
        ...
        cr->set_line_width(1.0);
        cr->set_fill_rule(Cairo::FILL_RULE_EVEN_ODD);
        for (std::vector<NavaidsDb::Navaid>::const_iterator i1(navaids.begin()), ie(navaids.end()); i1 != ie && !m_buffer.draw_checkabort(); i1++) {
                const NavaidsDb::Navaid& n(*i1);
                NavaidsDb::Navaid::navaid_type_t t(n.get_navaid_type());
		{
			bool ok(false);
			ok = ok || (m_buffer.draw_get_drawflags() & drawflags_navaids_ndb && NavaidsDb::Navaid::has_ndb(t));
			ok = ok || (m_buffer.draw_get_drawflags() & drawflags_navaids_vor && NavaidsDb::Navaid::has_vor(t));
			ok = ok || (m_buffer.draw_get_drawflags() & drawflags_navaids_dme && NavaidsDb::Navaid::has_dme(t));
			if (!ok)
				continue;
                }
                draw_navaid(cr, n, m_buffer.draw_transform(n.get_coord()));
        }
        cr->restore();
}
...
void VectorMapRenderer::DrawThreadOverlay::draw(Cairo::Context *cr, const std::vector<AirspacesDb::Airspace>& airspaces)
{
        ...
        //          << ((m_buffer.draw_get_drawflags() & drawflags_airspaces_specialuse) ? "SUAS " : "")
        //          << ((m_buffer.draw_get_drawflags() & drawflags_airspaces_ab) ? "AB" : "")
        //          << ((m_buffer.draw_get_drawflags() & drawflags_airspaces_cd) ? "CD" : "")
        //          << ((m_buffer.draw_get_drawflags() & drawflags_airspaces_efg) ? "EFG" : "") << std::endl;
        for (std::vector<AirspacesDb::Airspace>::const_iterator i1(airspaces.begin()), ie(airspaces.end()); i1 != ie && !m_buffer.draw_checkabort(); i1++) {
                const AirspacesDb::Airspace& a(*i1);
                // check for limit
                if (a.get_altlwr_corr() >= m_buffer.get_maxalt())
                        continue;
                if (a.get_typecode() == 255) {
                        // special use airspace
                        if (!(m_buffer.draw_get_drawflags() & drawflags_airspaces_specialuse))
                                continue;
                } else if (a.get_bdryclass() <= 'B') {
                        if (!(m_buffer.draw_get_drawflags() & drawflags_airspaces_ab))
                                continue;
                } else if (a.get_bdryclass() <= 'D') {
                        if (!(m_buffer.draw_get_drawflags() & drawflags_airspaces_cd))
                                continue;
                } else {
                        if (!(m_buffer.draw_get_drawflags() & drawflags_airspaces_efg))
                                continue;
                }
                Gdk::Color col = a.get_color();
                polypath(cr, a.get_polygon(), true);
                if ((m_buffer.draw_get_drawflags() & drawflags_airspaces_fill_ground) && is_ground(a)) {
                ...
                ScreenCoordFloat px(m_buffer.draw_transform(a.get_labelcoord()));
...
void VectorMapRenderer::DrawThreadOverlay::draw(Cairo::Context *cr, const std::vector<AirportsDb::Airport>& airports)
{
	...
	bool simplified(false);
	{
		...
		ScreenCoordFloat bottomright(to_screencoordfloat(m_buffer.draw_get_imagesize()));
		for (std::vector<AirportsDb::Airport>::const_iterator i1(airports.begin()), ie(airports.end()); i1 != ie && !m_buffer.draw_checkabort(); i1++) {
			const AirportsDb::Airport& a(*i1);
			bool ismil(false);
			switch (a.get_typecode()) {
                        case 'A':
                                if (!(m_buffer.draw_get_drawflags() & drawflags_airports_civ))
                                        continue;
                                break;

                        case 'C':
                                if (!(m_buffer.draw_get_drawflags() & drawflags_airports_mil))
                                        continue;
                                ismil = true;
                                break;
			...
			ScreenCoordFloat px(m_buffer.draw_transform(a.get_coord()));
		...
		if (simplified) {
			...
			continue;
                ...
                for (unsigned int i = 0; i < a.get_nrrwy(); i++) {
                        ...
                        if (!(m_buffer.draw_get_drawflags() & drawflags_airports_localizercone) || a.get_nrfinalapproachsegments())
                                continue;
                        if (rwy.is_he_usable()) {
                                ...
                                ScreenCoordFloat px(m_buffer.draw_transform(pt));
                                ScreenCoordFloat pxc(m_buffer.draw_transform(ptc));
                                ScreenCoordFloat pxs0(m_buffer.draw_transform(pts0));
                                ScreenCoordFloat pxs1(m_buffer.draw_transform(pts1));
		...
		if ((m_buffer.draw_get_drawflags() & drawflags_airports_localizercone) && a.get_nrfinalapproachsegments()) {
			...
			cr->begin_new_sub_path();
                        ...
                        double phi((rwydir - m_buffer.draw_get_upangle()) * FPlanLeg::to_rad);
...
{
        for (std::vector<WaypointsDb::Waypoint>::const_iterator i1(waypoints.begin()), ie(waypoints.end()); i1 != ie && !m_buffer.draw_checkabort(); i1++) {
                const WaypointsDb::Waypoint& w(*i1);
                switch (w.get_usage()) {
                        case WaypointsDb::Waypoint::usage_highlevel:
                                if (!(m_buffer.draw_get_drawflags() & drawflags_waypoints_high))
                                        continue;
                                break;

                        case WaypointsDb::Waypoint::usage_lowlevel:
                                if (!(m_buffer.draw_get_drawflags() & drawflags_waypoints_low))
                                        continue;
                                break;

                        case WaypointsDb::Waypoint::usage_bothlevel:
                                if (!(m_buffer.draw_get_drawflags() & (drawflags_waypoints_low | drawflags_waypoints_high)))
                                        continue;
                                break;

                        case WaypointsDb::Waypoint::usage_rnav:
                                if (!(m_buffer.draw_get_drawflags() & drawflags_waypoints_rnav))
                                        continue;
                                break;

                        case WaypointsDb::Waypoint::usage_terminal:
                                if (!(m_buffer.draw_get_drawflags() & drawflags_waypoints_terminal))
                                        continue;
                                break;

                        case WaypointsDb::Waypoint::usage_vfr:
                                if (!(m_buffer.draw_get_drawflags() & drawflags_waypoints_vfr))
                                        continue;
                                break;

                        default:
                        case WaypointsDb::Waypoint::usage_user:
                                if (!(m_buffer.draw_get_drawflags() & drawflags_waypoints_user))
                                        continue;
                                break;
                }
...
void VectorMapRenderer::DrawThreadOverlay::draw(Cairo::Context *cr, const std::vector<AirwaysDb::Airway>& airways)
{
        ...
        for (std::vector<AirwaysDb::Airway>::const_iterator i1(airways.begin()), ie(airways.end()); i1 != ie && !m_buffer.draw_checkabort(); i1++) {
                const AirwaysDb::Airway& awy(*i1);
                switch (awy.get_type()) {
                        case AirwaysDb::Airway::airway_high:
                                if (!(m_buffer.draw_get_drawflags() & drawflags_airways_high))
                                        continue;
                                break;

                        case AirwaysDb::Airway::airway_low:
                                if (!(m_buffer.draw_get_drawflags() & drawflags_airways_low))
                                        continue;
                                break;
                ...
                {
                        ScreenCoordFloat pb(m_buffer.draw_transform(awy.get_begin_coord()));
                        ScreenCoordFloat pe(m_buffer.draw_transform(awy.get_end_coord()));
...
void VectorMapRenderer::DrawThreadOverlay::draw_weather(Cairo::Context *cr)
{
	...
	{
		float sc(m_buffer.draw_get_scale());
		line2 = sc < 0.2;
		line1 = sc < 0.8;
		line1simple = sc >= 0.2;
	}
				...
				for (unsigned int linenr = 0; linenr < 2; ++linenr) {
					...
					case 0:
					{
						...
						if (line1simple) {
							if (!std::isnan(temp)) {
								float isa(0);
								IcaoAtmosphere<float>::std_altitude_to_pressure(0, &isa, m_buffer.draw_get_altitude() * Point::ft_to_m);
...
VectorMapRenderer::DrawThreadVMap::DrawThreadVMap(Engine& eng, Glib::Dispatcher& dispatch, const ScreenCoord& scrsize, const Point& center, int alt, uint16_t upangle, int64_t time)
        : DrawThreadOverlay(eng, dispatch, scrsize, center, alt, upangle, time)
{
}
...
void VectorMapRenderer::DrawThreadVMap::compute_pixmap(bool dbquery)
{
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
                dbquery = dbquery
			|| ((m_buffer.draw_get_drawflags() & drawflags_topo) && (!m_topocairoquery || m_topocairoquery->is_error()))
			|| ((m_buffer.draw_get_drawflags() & drawflags_terrain_all) && (!m_mapelementquery || m_mapelementquery->is_error()))
			|| need_dbquery();
        }
        TimeMeasurement tmquery;
        if (dbquery) {
		recompute_dbrectangle();
		std::cerr << "bbox: " << get_dbrectangle() << " (center " << m_buffer.draw_get_imagecenter() << ')' << std::endl;
                async_cancel();
                if (m_buffer.draw_get_drawflags() & drawflags_topo)
                        m_topocairoquery = m_engine.async_elevation_map_cairo(get_dbrectangle());
                if (m_buffer.draw_get_drawflags() & drawflags_terrain_all)
        ...
        // Wait until topo db is ready
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
		for (;;) {
			if (!m_topocairoquery || m_topocairoquery->is_done())
				break;
			if (m_buffer.draw_asyncwait_nolock())
				return;
		}
        ...
        // start drawing
        Cairo::RefPtr<Cairo::Context> cr = m_buffer.draw_create_context();
        cr->rectangle(0, 0, m_buffer.draw_get_imagesize().getx(), m_buffer.draw_get_imagesize().gety());
        cr->set_source_rgb(1.0, 1.0, 1.0);
        cr->fill_preserve();
        cr->clip();
        ...
        // draw topography
        if ((m_buffer.draw_get_drawflags() & drawflags_topo) && m_topocairoquery && m_topocairoquery->is_done() && !m_topocairoquery->is_error()) {
                std::cerr << "topo: cairo time " << tmquery << std::endl;
                tmquery.start();
        ...
        if ((m_buffer.draw_get_drawflags() & drawflags_terrain_all) && m_mapelementquery && m_mapelementquery->is_done() && !m_mapelementquery->is_error()) {
                std::cerr << "mapel: " << m_mapelementquery->get_result().size() << " time " << tmquery << std::endl;
                tmquery.start();
                ...
                if ((m_buffer.draw_get_drawflags() & drawflags_terrain_names) && (sz < 100) && !m_buffer.draw_checkabort()) {
                        cr->save();
                        cr->select_font_face("Sans", Cairo::FONT_SLANT_NORMAL, Cairo::FONT_WEIGHT_NORMAL);
                        cr->set_font_size(12);
                        cr->set_source_rgb(0.0, 0.0, 0.0);
...
void VectorMapRenderer::DrawThreadVMap::draw(Cairo::Context *cr, const MapelementsDb::Mapelement& mapel)
{
        ...
        switch (mapel.get_typecode()) {
			...
			if (!(m_buffer.draw_get_drawflags() & drawflags_terrain))
                                break;
                        cr->save();
                        cr->set_source_rgb(0.0, 0.0, 0.0);
                        cr->set_line_width(2.0);
			...
			if (!(m_buffer.draw_get_drawflags() & drawflags_terrain))
                ...
                case MapelementsDb::Mapelement::maptyp_border:
                        if (!(m_buffer.draw_get_drawflags() & drawflags_terrain_borders))
                                break;
                        cr->save();
...
void VectorMapRenderer::DrawThreadVMap::draw_text(Cairo::Context *cr, const MapelementsDb::Mapelement& mapel)
{
        ...
        ScreenCoord dim(m_buffer.draw_transform(bbox.get_northeast()) - m_buffer.draw_transform(bbox.get_southwest()));
...
void VectorMapRenderer::DrawThreadTerrain::compute_pixmap(bool dbquery)
{
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
                dbquery = dbquery
			|| (!m_topoquery || m_topoquery->is_error())
			|| ((m_buffer.draw_get_drawflags() & drawflags_navaids) && (!m_navaidquery || m_navaidquery->is_error()))
			|| ((m_buffer.draw_get_drawflags() & drawflags_waypoints) && (!m_waypointquery || m_waypointquery->is_error()))
			|| ((m_buffer.draw_get_drawflags() & drawflags_airports) && (!m_airportquery || m_airportquery->is_error()));
        }
        TimeMeasurement tmquery;
        ...
        // draw topography
        if (m_topoquery && m_topoquery->is_done() && m_topoquery->is_error()) {
		std::cerr << "topo: ERROR time " << tmquery << std::endl;
	}
        if (m_topoquery && m_topoquery->is_done() && !m_topoquery->is_error()) {
                std::cerr << "topo: time " << tmquery << std::endl;
                tmquery.start();
                ...
                if (surface) {
			int currentalt(m_buffer.draw_get_altitude());
                        unsigned int stride(surface->get_stride());
                        unsigned char *data(surface->get_data());
                        ...
                        std::cerr << "topo: draw time " << tmquery << std::endl;
			tmquery.start();
			draw_glidearea(surface);
                        draw_surface(cr.operator->(), m_topoquery->get_bbox(), surface, 1);
			std::cerr << "glide area: draw time " << tmquery << std::endl;
                }
        }
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
...
void VectorMapRenderer::DrawThreadAirportDiagram::compute_pixmap(bool dbquery)
{
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
                dbquery = dbquery
        ...
        // Wait until airport db is ready
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
		for (;;) {
			if (!m_airportquery || m_airportquery->is_done())
        ...
        // draw topography
        if (m_airportquery && m_airportquery->is_done() && !m_airportquery->is_error()) {
                std::cerr << "airport: time " << tmquery << std::endl;
                tmquery.start();
                const AirportsDb::elementvector_t& ev(m_airportquery->get_result());
                for (AirportsDb::elementvector_t::const_iterator ei(ev.begin()), ee(ev.end()); ei != ee && !m_buffer.draw_checkabort(); ++ei)
                        draw(cr.operator->(), *ei);
                std::cerr << "airport: draw time " << tmquery << std::endl;
        }
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
		for (;;) {
        ...
        if (m_navaidquery && m_navaidquery->is_done() && !m_navaidquery->is_error()) {
                std::cerr << "navaids: " << m_navaidquery->get_result().size() << " time " << tmquery << std::endl;
                tmquery.start();
                TimeMeasurement tm;
                const NavaidsDb::elementvector_t& ev(m_navaidquery->get_result());
                for (NavaidsDb::elementvector_t::const_iterator ei(ev.begin()), ee(ev.end()); ei != ee && !m_buffer.draw_checkabort(); ++ei)
                        draw(cr.operator->(), *ei);
                std::cerr << "navaids drawing time " << tm << std::endl;
	}
        {
        ...
        if (m_waypointquery && m_waypointquery->is_done() && !m_waypointquery->is_error()) {
                std::cerr << "waypoints: " << m_waypointquery->get_result().size() << " time " << tmquery << std::endl;
                tmquery.start();
                TimeMeasurement tm;
                const WaypointsDb::elementvector_t& ev(m_waypointquery->get_result());
                for (WaypointsDb::elementvector_t::const_iterator ei(ev.begin()), ee(ev.end()); ei != ee && !m_buffer.draw_checkabort(); ++ei)
                        draw(cr.operator->(), *ei);
                std::cerr << "waypoints drawing time " << tm << std::endl;
        }
	m_buffer.draw_done();
...
void VectorMapRenderer::DrawThreadAirportDiagram::polyline_to_path(Cairo::Context *cr, const AirportsDb::Airport::Polyline& pl)
{
        ...
        for (AirportsDb::Airport::Polyline::const_iterator ni(pl.begin()), ni0, ne(pl.end()); ni != ne; ni0 = ni, sc0 = sc1, ++ni) {
                ...
                sc1 = m_buffer.draw_transform(pn1.get_point());
                if (!pnfirst) {
                        pnfirst = &pn1;
                        scfirst = sc1;
                ...
                if (pn0.is_bezier_next() || pn1.is_bezier_prev()) {
                        ...
                        if (pn0.is_bezier_next())
                                scc0 = m_buffer.draw_transform(pn0.get_next_controlpoint());
                        ScreenCoordFloat scc1(sc1);
                        if (pn1.is_bezier_prev())
                                scc1 = m_buffer.draw_transform(pn1.get_prev_controlpoint());
                ...
                if (pn1.is_closepath()) {
                        if (pn0.is_bezier_next() || pn1.is_bezier_prev()) {
                                ...
                                if (pn1.is_bezier_next())
                                        scc0 = m_buffer.draw_transform(pn1.get_next_controlpoint());
                                ScreenCoordFloat scc1(scfirst);
                                if (pnfirst->is_bezier_prev())
                                        scc1 = m_buffer.draw_transform(pnfirst->get_prev_controlpoint());
...
void VectorMapRenderer::DrawThreadAirportDiagram::draw(Cairo::Context *cr, const AirportsDb::Airport& arpt)
{
        ...
        for (unsigned int r = 0; r < arpt.get_nrrwy(); ++r) {
                ...
                ScreenCoordFloat repx0(m_buffer.draw_transform(rept0));
                ScreenCoordFloat repx1(m_buffer.draw_transform(rept1));
                ScreenCoordFloat repx2(m_buffer.draw_transform(rept2));
                ScreenCoordFloat repx3(m_buffer.draw_transform(rept3));
                if (rwy.is_concrete())
                        cr->set_source_rgba(192.0/256.0, 192.0/256.0, 192.0/256.0, 1.0);
                else
                        cr->set_source_rgba(0.0/256.0, 150.0/256.0, 0.0/256.0, 1.0);
                ...
                // LE headend
                {
                        cr->save();
                        {
                                ...
                                ScreenCoordFloat pxcenter(m_buffer.draw_transform(ptcenter));
                                cr->translate(pxcenter.getx(), pxcenter.gety());
                        }
                        cr->rotate((rwy.get_le_hdg() + 0x8000 - m_buffer.draw_get_upangle()) * FPlanLeg::to_rad);
                ...
                // HE headend
                {
                        ...
                        cr->rotate((rwy.get_he_hdg() + 0x8000 - m_buffer.draw_get_upangle()) * FPlanLeg::to_rad);
                        //std::cerr << "scale: " << rwyhalfwidth << " width(nmi): " << (rwy.get_width() * (Point::ft_to_m / 2000.0f * Point::km_to_nmi)) << std::endl;
                        cr->scale(rwyhalfwidth, rwyhalfwidth);
                        cr->set_line_width(0.2);
                        cr->set_source_rgb(0.0, 0.0, 0.0);
                ...
                // draw centerline
                {
                        ...
                        ScreenCoordFloat px1(m_buffer.draw_transform(pt1));
                        ScreenCoordFloat px2(m_buffer.draw_transform(pt2));
        ...
        for (unsigned int r = 0; r < arpt.get_nrhelipads(); ++r) {
                ...
                ScreenCoordFloat px(m_buffer.draw_transform(hp.get_coord()));
        ...
        {
                cr->save();
                ScreenCoordFloat px(m_buffer.draw_transform(arpt.get_coord()));
                cr->translate(px.getx(), px.gety());
        ...
        for (unsigned int r = 0; r < arpt.get_nrpointfeatures(); ++r) {
                ...
                switch (pf.get_feature()) {
                        ...
                        {
                                ScreenCoordFloat px(m_buffer.draw_transform(pf.get_coord()));
                                cr->save();
                                cr->set_source_rgb(0.0, 0.0, 0.0);
                        ...
                        case AirportsDb::Airport::PointFeature::feature_vasi:
                        {
                                ...
                                cr->set_source_rgb(0.0, 0.0, 0.0);
                                cr->translate(px.getx(), px.gety());
                                cr->rotate((hdg - m_buffer.draw_get_upangle()) * FPlanLeg::to_rad);
                                cr->scale(featurescale, featurescale);
                        ...
                        case AirportsDb::Airport::PointFeature::feature_tower:
                        {
                                ...
                                cr->set_line_width(1.0);
                                cr->translate(px.getx(), px.gety());
                                cr->rotate((pf.get_hdg() - m_buffer.draw_get_upangle()) * FPlanLeg::to_rad);
                                cr->scale(featurescale, featurescale);
...
void VectorMapRenderer::DrawThreadAirportDiagram::draw(Cairo::Context *cr, const NavaidsDb::Navaid& n)
{
        ...
        cr->set_line_width(1.0);
        cr->set_fill_rule(Cairo::FILL_RULE_EVEN_ODD);
        ScreenCoordFloat pos(m_buffer.draw_transform(n.get_coord()));
        cr->translate(pos.getx(), pos.gety());
        float featurescale(nmi_to_pixel(Point::km_to_nmi * 0.002));
        cr->scale(featurescale, featurescale);
...
VectorMapRenderer::DrawThreadTMS::DrawThreadTMS(Engine& eng, Glib::Dispatcher& dispatch, unsigned int source, const ScreenCoord& scrsize, const Point& center, int alt, uint16_t upangle, int64_t time)
        : DrawThreadOverlay(eng, dispatch, scrsize, center, alt, upangle, time), m_cache(source)
{
...
void VectorMapRenderer::DrawThreadTMS::compute_pixmap(bool dbquery)
{
	m_cache.clear_queue();
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
	...
	{
		m_cache.clear_queue();
		unsigned int zoom(TileIndex(tilesources[m_cache.get_source()].idxmode).nearest_zoom(m_buffer.draw_get_scale(), m_buffer.draw_get_imagecenter()));
		...
		Rect bbox(m_buffer.draw_coverrect(1.1, 1.1));
		idx0.set_rounddown(bbox.get_southwest());
		idxe.set_roundup(bbox.get_northeast());
		if (true)
			std::cerr << "DrawThreadTMS::draw_iterate: center " << m_buffer.draw_get_imagecenter()
				  << " scale " << m_buffer.draw_get_scale() << " tilescale "
				  << TileIndex(tilesources[m_cache.get_source()].idxmode).nmi_per_pixel(zoom, m_buffer.draw_get_imagecenter())
...
VectorMapRenderer::DrawThreadBitmap::DrawThreadBitmap(Engine& eng, Glib::Dispatcher& dispatch, const Glib::RefPtr<BitmapMaps::Map>& map, const ScreenCoord& scrsize, const Point& center, int alt, uint16_t upangle, int64_t time)
        : DrawThreadOverlay(eng, dispatch, scrsize, center, alt, upangle, time), m_map(map)
{
}
...
void VectorMapRenderer::DrawThreadBitmap::compute_pixmap(bool dbquery)
{
        {
                Glib::Mutex::Lock lock(m_buffer.get_mutex());
                dbquery = dbquery || need_dbquery();
	...
	// draw bitmap
	if (m_map) {
		Rect bbox(m_buffer.draw_coverrect(2, 2));
