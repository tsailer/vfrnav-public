#ifndef CFMUAUTOROUTEPROXY_H
#define CFMUAUTOROUTEPROXY_H
...
class CFMUAutorouteProxy : public sigc::trackable {
public:
	typedef enum {
		proxystate_stopped,
		proxystate_started,
	...
	CFMUAutorouteProxy();
	~CFMUAutorouteProxy();
	void start_autorouter(void) { child_run(); }
	void stop_autorouter(void) { child_close(); }
	bool is_autorouter_running(void) const { return m_childrun; }

	void clear_fplan(void);
	FPlanRoute& get_fplan(void) { return m_route; }
	const FPlanRoute& get_fplan(void) const { return m_route; }
	void set_fplan(const FPlanRoute& fpl);
...
protected:
	...
	sigc::connection m_connchildstdin;
	...
	bool child_stdin_handler(Glib::IOCondition iocond);
	bool child_stdout_handler(Glib::IOCondition iocond);
	void recvcmd(const Command& cmd);
};

extern const Glib::ustring& to_str(CFMUAutorouteProxy::proxystate_t ps);

#endif /* CFMUAUTOROUTEPROXY_H */
